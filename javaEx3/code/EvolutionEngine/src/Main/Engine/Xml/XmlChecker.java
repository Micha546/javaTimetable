...
public class XmlChecker {
    ...
    public static void CheckDescriptor(ETTDescriptor descriptor)
    {
        ...
        /*AssertElitismIsSmallerThanInitialPopulation(descriptor);*/
        AssertWorkingHoursIsLessThanDaysHours(descriptor);
    }
    ...
    /*private static void AssertElitismIsSmallerThanInitialPopulation(ETTDescriptor descriptor)
    {
        int elitism = descriptor.getETTEvolutionEngine().getETTSelection().getETTElitism();
        int initialPopulationSize = descriptor.getETTEvolutionEngine().getETTInitialPopulation().getSize();

        if(elitism < 0)
            throw new RuntimeException("The elitism value in the file is illegal. elitism=" +
                    elitism + " (must be greater than 0)");
        else if(elitism > initialPopulationSize)
            throw new RuntimeException("The elitism value in the file is illegal. elitism=" +
                    elitism + " initial population size=" + initialPopulationSize +
                    " (elitism must be smaller than initial population size)");
    }*/

    private static void AssertWorkingHoursIsLessThanDaysHours(ETTDescriptor descriptor)
    {
        int DH = descriptor.getETTTimeTable().getDays() * descriptor.getETTTimeTable().getHours();

        List<ETTTeacher> ettTeachers = descriptor.getETTTimeTable().getETTTeachers().getETTTeacher();

        for(ETTTeacher ettTeacher : ettTeachers)
            if(ettTeacher.getETTWorkingHours() > DH)
                throw new RuntimeException("The teacher " + ettTeacher.getETTName() + " has more working hours than DH");
    }
}
