...
public enum TimeTableRules implements Rule {
    ...
    DayOffClass{
        ...
        protected double getEnforcementTimeTable(TimeTable timeTable, TimeTableManager manager, Object... args) {
            ...
            Map<Grade, Boolean> gradeToHasDayOff = manager.getAllGrades().stream()
                    .collect(Collectors.toMap(grade -> grade,
                            grade -> {
                                Set<Integer> daysGradeIsStudying = timeTable.getTuples().stream()
                                        .filter(tuple -> tuple.getGrade() == grade)
                                        .mapToInt(TimeTableTuple::getDay)
                                        .boxed()
                                        .collect(Collectors.toSet());
                                ...
                                return IntStream.range(1, timeTable.getDaysLimit())
                                        .anyMatch(day -> !daysGradeIsStudying.contains(day));
                            }));

            return (double) gradeToHasDayOff.values().stream().filter(value -> value).count()
                    / (double) gradeToHasDayOff.size();
        }
    },
    WorkingHoursPreference{
        ...
        protected double getEnforcementTimeTable(TimeTable timeTable, TimeTableManager manager, Object... args) {
            ...
            Map<Teacher, Integer> teacherToActualHoursTeaching = manager.getAllTeachers().stream()
                    .collect(Collectors.toMap(
                            teacher -> teacher,
                            teacher -> (int) timeTable.getTuples().stream()
                                    .filter(tuple -> tuple.getTeacher() == teacher)
                                    .count()
                    ));

            return teacherToActualHoursTeaching.entrySet().stream()
                    .mapToDouble(entry -> {
                        int prefWorkingHoursOfTeacher = entry.getKey().getWorkingHours();
                        int actualWorkingHoursOfTeacher = entry.getValue();

                        if(prefWorkingHoursOfTeacher == actualWorkingHoursOfTeacher)
                            return 1;
                        else if(prefWorkingHoursOfTeacher > actualWorkingHoursOfTeacher)
                            return (double) actualWorkingHoursOfTeacher / (double) prefWorkingHoursOfTeacher;
                        else
                            return (double) prefWorkingHoursOfTeacher / (double) actualWorkingHoursOfTeacher;
                    })
                    .sum() / (double) teacherToActualHoursTeaching.size();
        }
    };
